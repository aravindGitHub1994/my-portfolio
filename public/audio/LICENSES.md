...
### Tier 2/3 (§6.2) — sheddable garnish

These sit on the `texture` and `music` buses, which 7.2's watchdog drops
before touching anything above.

| Sound | Bus | Synthesis |
|---|---|---|
| Key clack | texture | Triangle knock 150→70 Hz + band-passed switch click |
| Drive seek chatter | texture | 2–4 band-passed noise bursts, seeded spacing |
| Dusk fan bed | texture | Looped noise through a 190 Hz low-pass |
| Egg stinger (awaits 8.x) | machine | Three FM bells rising, D6–G6–C7 |
| Earbud music leak | music | **Original figure** — sparse A-minor contour on a triangle, hard 900 Hz low-pass |

The earbud leak is deliberately a contour rather than a tune: what
escapes someone else's earbuds is shape, not melody.

The startup chime and the earbud-leak figure are both original
compositions written for this project. The chime is intended to evoke the
era's optimism, not to resemble any particular product's chime.
